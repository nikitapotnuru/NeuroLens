...
export default function ResultsPage() {
  ...
  // 🟢 Handles generating the PDF report
  const handleDownloadReport = () => {
    window.print();
  };

  return (
    // print:bg-white ensures the background is plain white in the PDF
    <div className="min-h-screen bg-gray-50 print:bg-white flex flex-col items-center py-10 px-4 sm:px-6 font-sans">
      <div className="max-w-5xl w-full">
        
        {/* PDF ONLY HEADER (Hidden on website, visible only on PDF) */}
        <div className="hidden print:flex items-center justify-between border-b-2 border-gray-200 pb-4 mb-8">
          <div className="flex items-center gap-2 text-nl-primaryTeal font-extrabold text-2xl">
            <Activity className="w-8 h-8" />
            <span>NeuroLens ASD Screening Tool</span>
          </div>
          <div className="text-right text-gray-500 text-sm">
            <p>Official AI Analysis Report</p>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 print:mb-4">
          <div>
            <h1 className="text-3xl font-extrabold text-nl-darkText flex items-center gap-3">
              <FileText className="w-8 h-8 text-nl-primaryTeal print:hidden" /> Session Report
            </h1>
            <p className="text-gray-500 mt-1">Child: <span className="font-bold text-gray-800">Leo (18 mos)</span> • Session: #NL-882</p>
            <p className="text-gray-500 mt-1">References guidelines from the WHO and CDC.</p>
          </div>
          
          {/* Buttons hidden during PDF generation */}
          <div className="flex gap-3 print:hidden">
            <button 
              onClick={handleDownloadReport}
              className="flex items-center gap-2 bg-nl-primaryTeal text-white px-5 py-2.5 rounded-full font-bold shadow-md hover:bg-teal-600 transition-all"
            >
        ...
        <div className={`w-full p-8 rounded-[2rem] shadow-sm border mb-8 flex flex-col md:flex-row items-center justify-between print:shadow-none print:border-2 ${thresholdMet ? 'bg-orange-50 border-orange-200' : 'bg-green-50 border-green-200'}`}>
          <div className="flex items-center gap-6 mb-6 md:mb-0">
            <div className={`w-28 h-28 rounded-full flex flex-col items-center justify-center bg-white shadow-md border-4 print:shadow-none ${thresholdMet ? 'border-orange-400' : 'border-green-400'}`}>
          ...
          <div className="max-w-md bg-white/70 p-5 rounded-2xl border border-white text-sm text-gray-700 shadow-sm leading-relaxed print:shadow-none print:border-gray-300">
            ...
            Because the subject's behavioral data aligns <b>{autismAlignmentPercentage}%</b> with the autistic dataset (which is &ge; 50%), there are chances the subject is exhibiting traits associated with ASD. <br/><br/><i>Consult a pediatrician for formal diagnosis.</i>
          </div>
        </div>

        {/* 🟢 3-WAY DATA COMPARISON VISUALIZATION */}
        <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 mb-8 w-full flex flex-col items-center print:shadow-none print:border-2">
        ...
        {/* PDF ONLY FOOTER */}
        <div className="hidden print:block text-center text-xs text-gray-400 mt-10 border-t pt-4">
